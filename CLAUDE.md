...
| PUT | `/api/admin/groups/{id}/training-goal` | Admin | Vorlage setzen/aktualisieren (DailyMinutes 0–600 = ein gemeinsames Tageszeit-Ziel, PlayGames 0–200 Partien/Woche, WeeklyDaysTarget 0–7) |
...
### Trainingsziele (auth)
**Ein** gemeinsames Tageszeit-Ziel `DailyMinutes` (alle Quellen füttern denselben Topf) + wöchentliches Spielen-Ziel (Anzahl Rapid-/Classical-Partien pro ISO-Woche) + Wochenziel (volle Tage); effektives Ziel = persönlicher Override > zuletzt aktualisierte Gruppen-Vorlage > keins. Tracker aggregiert je UTC-Tag die verbrachte Trainingszeit (Pro-Einzelpuzzle-Clamp 1800 s, Endlos/Manuell-Clamp 14400 s, Chessable-Häppchen-Clamp 3600 s) und markiert Tage none/partial/full gegenüber dem einen Tageszeit-Ziel (Spielen ist ein separates Wochenziel, füttert den Zeit-Topf NICHT). Die Zeit wird zusätzlich **nach Quelle** und **nach Thema** aufgezeichnet (für „Heute" + Perioden-Summen im Tracker): Quellen = `randomPuzzle` (PuzzleAttempt + EndlessSession + BookPuzzleAttempt + manuell OfflinePuzzle), `courseBook` (alle CourseAttempt, beide `Book.Kind`, + manuell OfflineStudy/Coaching), `chessable` (ChessableActivity, RepCheck via `POST /api/extension/training-activity`). Themen = Eröffnung/Mittelspiel/Endspiel/Taktik best-effort (Standard-Puzzle: `Puzzle.Themes`-Phasentag sonst Taktik; Buch/Kurs: `BookPuzzle.Tags`/`Chapter` sonst Taktik bzw. Studienbuch→`other`; Chessable: `CourseKind`; Endless→Taktik), sonst **`other` (Sonstiges)** — die Summe je Quelle bzw. je Thema ergibt die Gesamtzeit. Logik in `TrainingGoalService`; Admin-Vorlage je Gruppe siehe Gruppen-Tabelle.

**Manuelle Offline-Aktivitäten** (selbst gemeldet, korrigierbar): `ManualActivities` (`/api/training-goals/manual` GET/POST/PUT/DELETE) speist denselben Zeit-Topf bzw. das Spielen-Ziel. Mapping je `ManualActivityKind`: **OtbGame** → Spielen (+Amount Partien/Tag, Cap 50, kein Zeit-Topf), **OfflinePuzzle** → Quelle randomPuzzle/Thema Taktik (Amount Min), **OfflineStudy** + **Coaching** → Quelle courseBook/Thema other (Amount Min); Minuten-Arten via `PerSessionCapSeconds` (4 h) gedeckelt. Tage mit ≥1 manuellem Eintrag liefern `TrackerDayDto.HasManual=true` (Tracker-Marker „manuell").
...
| PUT | `/api/training-goals` | Auth | Persönlichen Override setzen (DailyMinutes 0–600, PlayGames 0–200 Partien/Woche, WeeklyDaysTarget 0–7) |
| DELETE | `/api/training-goals` | Auth | Override entfernen → Rückfall auf Gruppen-Vorlage |
| GET | `/api/training-goals/today` | Auth | Heutiger Fortschritt (Tageszeit-Ziel `daily` + Aufschlüsselung `bySource`/`byTheme`) + Spielen-Partien (Woche) + Tagesstatus + Wochenstand (X/Y Tage) |
| GET | `/api/training-goals/tracker?weeks=27` | Auth | Tagesreihe (nur Tage mit Aktivität; je Tag `totalSeconds`+`bySource`+`byTheme`+PlayGames) + Perioden-Summen `breakdownBySource`/`breakdownByTheme` |
...
| GroupTrainingGoals | Coach-Vorlage Trainingsziel je Gruppe | GroupId (unique, Cascade von Group), DailyMinutes (gemeinsames Tageszeit-Ziel), PlayGames (Partien/Woche), WeeklyDaysTarget, CreatedAt, UpdatedAt |
| UserTrainingGoals | Persönlicher Trainingsziel-Override | UserId (unique, Cascade), DailyMinutes (gemeinsames Tageszeit-Ziel), PlayGames (Partien/Woche), WeeklyDaysTarget, CreatedAt, UpdatedAt |
