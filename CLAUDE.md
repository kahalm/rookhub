...
- **Aktuelle Version**: `0.181.2` — 0.181.2 (interne Aufräumarbeit: Endless-Fasttrack-Schwellen-Logik in eigene unit-getestete State-Klasse `endless-fasttrack-state.ts` ausgelagert); 0.181.1 (interne Aufräumarbeit: app-root Router-/Service-Worker-Subscriptions via `takeUntilDestroyed`); 0.181.0 (Kurs-Zufallsmodus: jedes Puzzle nur einmal pro Durchgang); vollständiger Verlauf ausschließlich in `src/frontend/app/src/environments/changelog.ts` (Single Source)
