...
public class RepertoireControllerTests : IDisposable
{
    ...
    public RepertoireControllerTests()
    {
        ...
        _controller = new RepertoireController(_service, ReprocessTestHelper.Build(_db), new RecordingReprocessLauncher(), new RepertoireTrainingService(_db), courseService, new SharedLineService(_db), new RepertoirePositionLookupService(_db, cache), new FlashcardMarkService(_db));
    }
