...
public class BookPuzzleControllerTests : IDisposable
{
    ...
    public BookPuzzleControllerTests()
    {
        ...
        _controller = new BookPuzzleController(new BookPuzzleService(_db, NullLogger<BookPuzzleService>.Instance, new NoOpTaskQueue()));
        SetUser(99);
    }
    ...
    public async Task RecordAttempt_LogsStartAndSolveTime()
    {
        ...
        var controller = new BookPuzzleController(new BookPuzzleService(_db, logger, new NoOpTaskQueue())) { ControllerContext = _controller.ControllerContext };
