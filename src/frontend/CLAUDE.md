...
- `fullscreen-overlay.service.ts` — hängt den CDK-Overlay-Container während des Vollbilds INS Vollbild-Element (app-weit in `AppComponent` instanziiert). Ohne das sind Dialog/Snackbar/Menü/Tooltip im Vollbild unsichtbar; ein modaler Dialog mit `disableClose` (Lösezeit-Nachfrage) hing die App fest. Beim Verlassen — und wenn das Vollbild-Element zerstört wurde — wandert der Container zurück ans `<body>`.
