...
- `board-fullscreen-button.component.ts` — der Knopf in der Brett-Ecke. Sitzt INNERHALB des Vollbild-Elements, damit man im Vollbild ohne Tastatur wieder herauskommt; erklärt sich per nativem `title` (CDK-Overlays hängen am `<body>` und werden im Vollbild nicht gerendert).
- Eingebaut in alle drei Brett-Komponenten (`PuzzleBoardComponent`, `AnalysisBoardComponent`, `pgn-viewer/ChessBoardComponent`) → jedes Brett der App hat es. Ins Vollbild geht immer der Brett-**Wrapper**, per `:fullscreen`-CSS ein zentriertes `min(100vw,100vh)`-Quadrat (`!important`, da die UA-Regeln `!important` setzen) — sonst verrutschen die absolut positionierten Auflagen (Umwandlungs-Auswahl, Viz-Ring).
