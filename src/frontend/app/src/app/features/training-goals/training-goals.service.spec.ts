...
describe('TrainingGoalService', () => {
  ...
  it('gets the effective goal', () => {
    ...
    req.flush({ dailyMinutes: 25, playGames: 0, weeklyDaysTarget: 5, source: 'group', groupName: 'A' });
  });

  it('saves a personal override via PUT', () => {
    svc.saveGoal({ dailyMinutes: 30, playGames: 0, weeklyDaysTarget: 0 })
      .subscribe(g => expect(g.source).toBe('personal'));
    const req = http.expectOne('/api/training-goals');
    expect(req.request.method).toBe('PUT');
    expect(req.request.body.dailyMinutes).toBe(30);
    req.flush({ dailyMinutes: 30, playGames: 0, weeklyDaysTarget: 0, source: 'personal', groupName: null });
  });
  ...
  it('deletes the personal override via DELETE', () => {
    ...
    req.flush({ dailyMinutes: 0, playGames: 0, weeklyDaysTarget: 0, source: 'none', groupName: null });
  });
