...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  { version: "0.201.0", date: "2026-06-29", changes: [
    { en: "Course \"Whole book\": the random-mode end screen no longer claims \"Course completed!\" when only the current round's pool is exhausted but failed puzzles still remain. It now says \"Random round finished — N puzzles still to solve\" with a \"Continue with remaining\" button (resets to bring them back). Only when every puzzle is actually solved does the trophy + \"Course completed!\" message appear.", de: "Kurs „Ganzes Buch\": Der Zufallsmodus-Abschluss-Bildschirm meldet nicht mehr „Kurs abgeschlossen!\", wenn nur der Pool der aktuellen Runde leer ist, aber gescheiterte Aufgaben übrig sind. Stattdessen steht „Zufalls-Runde durch — noch N Aufgaben zu lösen\" mit „Mit den restlichen weitermachen\"-Knopf (Reset bringt sie zurück). Die Trophäe + „Kurs abgeschlossen!\" erscheinen nur, wenn wirklich alle Puzzles gelöst sind." },
    { en: "Repertoire trainer reworked: a playable-but-not-repertoire move no longer reveals the repertoire move — it only says \"playable move, find the repertoire one next round\". A wrong move no longer instantly shows the solution either — you get \"Mouseslip\" (try again, no penalty) and \"Show solution\" buttons. \"Show solution\" reveals the move and counts the card as wrong (re-queued at the end of the session).", de: "Repertoire-Trainer überarbeitet: Spielbare-aber-nicht-Repertoire-Züge verraten den Repertoire-Zug nicht mehr — es steht nur „spielbarer Zug, finde den Repertoire-Zug in der nächsten Runde selbst\". Ein falscher Zug zeigt die Lösung ebenfalls nicht mehr sofort — du bekommst „Mausrutscher\" (nochmal versuchen, keine Wertung) und „Lösung zeigen\". „Lösung zeigen\" enthüllt den Zug und zählt die Karte als falsch (kommt am Sitzungsende erneut)." },
    { en: "Repertoire trainer: after a wrong move, Stockfish 18 evaluates both your move and the repertoire move in the background, and the feedback shows how much worse your choice is (\"Your move is 1.4 pawns worse than the repertoire move\"). Forced-mate situations get explicit warnings (\"Repertoire had a forced mate — your move misses it\" / \"Your move allows a forced mate for the opponent\").", de: "Repertoire-Trainer: Nach einem falschen Zug bewertet Stockfish 18 deinen Zug und den Repertoire-Zug im Hintergrund und zeigt dir, wie viel schlechter deine Wahl ist („Dein Zug ist um 1,4 Bauern schlechter als der Repertoire-Zug\"). Erzwungene Matt-Situationen bekommen explizite Hinweise („Im Repertoire war ein erzwungenes Matt — dein Zug verpasst es\" / „Dein Zug erlaubt dem Gegner ein erzwungenes Matt\")." },
  ]},
  ...
  { version: "0.178.1", date: "2026-06-23", changes: [
    { en: "Android app: fixed the 'Chrome required' dialog on devices without Chrome — the app now falls back to the system WebView instead.", de: "Android-App: Behebt den „Chrome erforderlich“-Dialog auf Geräten ohne Chrome — die App fällt jetzt auf die System-WebView zurück." },
  ]},
  ...
  { version: "0.176.1", date: "2026-06-22", changes: [
    { en: "Security: deleting your account now immediately blocks any still-valid login token and revokes your API tokens — a deleted account can no longer access the API with an old token.", de: "Sicherheit: Beim Löschen des Kontos werden jetzt sofort noch gültige Login-Token blockiert und API-Tokens widerrufen — ein gelöschtes Konto kann die API nicht mehr mit einem alten Token nutzen." },
  ]},
  ...
  { version: "0.167.0", date: "2026-06-21", changes: [
    { en: "The \"did it really take that long?\" prompt for solves over 5 minutes now works in all puzzle modes — standard and endless too, not just book/course. If you were away, the recorded time is capped at 5 minutes everywhere.", de: "Die Nachfrage „hat das wirklich so lange gedauert?“ bei Lösungen über 5 Minuten greift jetzt in allen Puzzle-Modi — auch Standard und Endlos, nicht nur Buch/Kurs. Warst du weg, wird die gewertete Zeit überall auf 5 Minuten gekappt." },
  ]},
  ...
  { version: "0.155.14", date: "2026-06-18", changes: [
    { en: "Endless history list is lighter: each row's config, mistakes and Elo-change text is now computed once when the page loads instead of re-parsing JSON on every screen update.", de: "Die Endless-History-Liste ist leichtgewichtiger: Konfig, Fehler und Elo-Veränderung je Zeile werden jetzt einmal beim Laden berechnet, statt bei jedem Bildschirm-Update das JSON neu zu parsen." },
  ]},
