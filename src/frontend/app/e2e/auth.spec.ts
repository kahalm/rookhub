...
test.describe('Auth', () => {
  test('login page shows form', async ({ page }) => {
    ...
    await expect(page.locator('mat-card').getByRole('button', { name: /login|anmelden/i })).toBeVisible();
  });

  test('login with valid credentials redirects to dashboard', async ({ page, testUser }) => {
    await page.goto('/login');

    const form = page.locator('mat-card');
  ...
  test('navbar shows username after login', async ({ authedPage }) => {
    const { username } = JSON.parse(fs.readFileSync(path.join(__dirname, '.auth-state.json'), 'utf-8'));
    await authedPage.goto('/dashboard');
    await authedPage.waitForURL('**/dashboard', { timeout: 10_000 });

    // Welcome message contains username
    await expect(authedPage.locator('h1')).toContainText(username, { timeout: 10_000 });
  });
});
