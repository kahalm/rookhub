...
- [~] **`OnPush` ausrollen** → **begonnen v0.184.20/0.184.22**: präsentationale Komponenten (loading-spinner, puzzle-tags, theme-picker, review-nav, promotion-picker) auf OnPush. OFFEN: weitere reine Display-Cards mit Objekt-/Array-Inputs (erst In-place-Mutation der Eltern prüfen) + die wertvollen, aber risikoreichen Solver/Analyse/Turnier-Tabellen (Timer via `NgZone.runOutsideAngular`).
