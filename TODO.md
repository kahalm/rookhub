...
- [ ] **RepCheck: gespeicherter rkh_-Token wird ins seiten-injizierte Input vorbefüllt** — `extension/content.js:1131-1135` (+ userscript `:1260`) setzt `tokenInput.value = cfg.token` in ein in die chess.com/lichess-Seite injiziertes `<input type=password>` → per MutationObserver/XSS auf der Seite auslesbar (untergräbt die v1.19.1-Härtung „Token nie seiten-lesbar"). Fix: Feld nie prefillen, nur bei Neu-Eingabe überschreiben.
- [ ] **RepCheck: Chessable-Bearer plaintext in `chrome.storage.local` + wird bei Chessable-Logout nicht gelöscht** — `extension/chessable-token.js:41-48`/`30-49` (syncToken returnt bei null früh, löscht alten Token nicht → bleibt bis Server-Ablauf). At-rest nicht schlimmer als Chessables eigenes localStorage; Fix: bei Extraktions-Fehler `remove('chessableToken')` + TTL; host_permissions `https://*/*` via optional_permissions auf die RookHub-Origin verengen.
...
- [ ] **Leaderboard `alltime` (Default) materialisiert ganze Daily-Attempt-Tabelle im RAM** — `Services/LeaderboardService.cs:91-107` (Regel „Attempt-Datum == Daily-Zuordnung" nicht SQL-übersetzt). Bei ~49 Usern ok, wächst unbounded. Fix: Aggregation in die DB (`GROUP BY UserId,Date` + Join auf Zuordnungsdatum). (= bekannter „Revisit"-Punkt, jetzt priorisiert.)
...
- [ ] **Crawler: 429/5xx ohne Retry-After/Backoff** → 1 flacher Retry (5s) dann harter Job-`Failed` (`CrawlerService.cs:658-676,785-809`). Fix: Retry-After honorieren + gestaffelter Backoff. (VPN-Rotation-Restart läuft weiter im Rate-Limit-Lock `:927-957/686-697` — nur das IP-Polling ist off-lock; Rotation-DoS-Fenster ≤ ~3s.)
- [ ] **piratechess `POST /api/vpn/rotate` nur `[Authorize]` + offene Registrierung** (`VpnController.cs:47-53`, `AuthController.cs:18-26`) → jeder Selbst-Registrierte kann die geteilte Exit-IP rotieren (Import-DoS). Fix: `[ServiceKeyAuth]` auf /rotate; Registrierung schließen ODER prod-Host-Port-Mapping droppen (rookhub spricht ohnehin über die Bridge). Dazu: direct/* ohne Rate-Limiter/`RequestSizeLimit`; Container läuft als root (kein `USER`).
- [ ] **rookhub: SharedLine erlaubt beliebigen öffentlichen Text + überlebt Share-Widerruf + kein Size-Cap** — `Services/SharedLineService.cs:28-42,62-101`: `dto.Pgn` wird nie gegens Repertoire geprüft (nur `LooksLikePgn`) → beliebiger Text unter `/l/{token}`; Empfänger darf Links anlegen, Snapshot überlebt RepertoireShare-Widerruf; kein `[RequestSizeLimit]`. Fix: Länge cappen, `CreateAsync` auf Owner beschränken bzw. bei Widerruf löschen.
- [ ] **rookhub: `ChessableController` klassenweit `[DisableRateLimiting]`** (`:28`) deckt auch `POST /test` + `GET /courses?refresh=true` (live piratechess→Chessable über geteilten VPN-Pool) → Loop kann die IP verbrennen/Cloudflare-Block. Fix: Polling-GETs frei lassen, per-User-Limit (5/min) auf test + refresh.
- [ ] **rookhub: `WeeklyPostController` ohne Klassen-`[Authorize]`** (`:19-21`) — heute alle Actions explizit attributiert (kein Live-Gap), aber offener Default = Falle für künftige Actions inkl. der Admin-Absolut-Routen in derselben Klasse. Fix: Klassen-`[Authorize]` + gezielt `[AllowAnonymous]`.
- [ ] **log-watcher: Markdown-Injection in Discord-Alert** — `signal.detail` (aus `url.path`/Hostnamen, `security.py:62-67`/`linux.py`) wird ungescrubbt ins Embed geschrieben (`main.py:243`/`discord_notify.py`); Angreifer-Pfad `/[x](https://evil)` → klickbarer Link im Admin-Alert (kein Ping, nur Link-Spoof). Digest-Pfad (`main.py:373`, `post_text`) parst `@everyone` → `allowed_mentions:{parse:[]}` setzen + detail scrubben.
- [ ] **schach-bot: `GET /webhook/build-info` unauth** (`core/webhook_server.py:262-267`, Port host-published) verrät die laufende Commit-SHA. Fix: hinter HMAC/Bearer ODER Port nicht publishen. (HMAC-Replay ohne Timestamp-Header nur im Backward-Compat-Zweig — sobald rookhub immer den Timestamp sendet, Pflicht machen; gilt analog `BotStatsController.cs:72-100`.)
...
- [ ] **~0,8 MB Changelog im Initial-Bundle (MITTEL, Performance — größter Hebel)**: `environments/changelog.ts` ist **811 KB** Quelltext (zweisprachige Prosa je Version) und wird über `environment.ts`/`environment.prod.ts` **eager** importiert → landet nachweislich in `main-*.js` (1,49 MB roh / 468 KB gzip; Changelog-Strings im main-Chunk verifiziert). Jeder Erstaufruf lädt die komplette Versionshistorie, obwohl sie nur das Changelog-Overlay braucht; das Initial-Budget wurde deshalb schon auf 1,5/2 MB angehoben und wächst mit jedem Release weiter. Fix: `APP_VERSION` (+`APK_VERSION`) in ein winziges Modul, `CHANGELOG` per `await import('./changelog-data')` erst beim Öffnen des Overlays (oder als JSON-Asset `fetch`en, dann auch SW-cachebar). Erwartung: Initial-Bundle grob halbiert.
- [ ] **„Kurs offline gespeichert" wird gemeldet, obwohl der Cache fehlschlug (MITTEL, Korrektheit)**: `saveBookOffline` (`features/puzzles/book-offline.util.ts:20`) verschluckt einen Quota-Fehler und liefert `void`; `course-list.component.ts:367` zeigt danach unbedingt `courses.offlineSaved` und setzt `offlineFiles.add(...)`. Bei vollem localStorage (große Bücher = mehrere MB) glaubt der User, der Kurs sei offline verfügbar — er ist es nicht, und nach dem Reload springt das ☁-Häkchen zurück (Zustand kommt dann aus `cachedBookFileNames()`). Fix: `boolean` zurückgeben und im Fehlerfall `courses.offlineFailed` zeigen (der Key existiert bereits).
- [ ] **Quadratisches Schreiben beim anonymen Kurs-Cache (MITTEL, Performance)**: `book-puzzle.component.ts:888` schreibt nach JEDER geladenen Seite den **wachsenden** Akkumulator komplett ins localStorage (`ANON_COURSE_PAGE_SIZE = 300`). Bei 3000 Puzzles sind das 10 synchrone `JSON.stringify`+`setItem` über 300/600/…/3000 Einträge ≈ 16.500 statt 3.000 serialisierte Puzzles — mehrere MB Main-Thread-Blockade auf Mobilgeräten. Fix: einmal nach der ERSTEN Seite (schneller Offline-Start) und dann nur noch am Ende (oder alle k Seiten).
- [ ] **Ungeschütztes `JSON.parse` aus localStorage (LOW, Robustheit)**: `tournament-list.component.ts:144` — `new Set(JSON.parse(stored))` ohne `try` (einzige der 46 Parse-Stellen ohne Guard; wirft auch bei gültigem Nicht-Array). Ein beschädigter `hiddenTournaments`-Wert wirft in `ngOnInit` → Turnierseite dauerhaft tot, bis der User localStorage manuell leert.
- [ ] **Offline-Queue: stiller Verlust + unbegrenztes Wachstum (LOW)**: `core/offline-queue.service.ts:130` verschluckt Quota-Fehler beim Schreiben → eine offline gemerkte Lösung kann spurlos verschwinden (`enqueue` prüft den Erfolg nicht); die Queue hat zudem kein Limit. Fix: Schreiberfolg prüfen + einmalig warnen, Länge deckeln (z. B. 500, ältester fliegt mit Log).
- [ ] **`reviewGoTo` 3× dupliziert, 2× ohne FEN-Guard (LOW, Robustheit + Duplikation)**: `puzzle.component.ts:557` und `endless-puzzle.component.ts:1222` sind Zeile-für-Zeile-Kopien der Buch-Variante, aber mit `new Chess(this.puzzle.fen)` statt `tryLoadFen` — also genau die Klasse, die in 0.316.3/0.317.2 zweimal zugeschlagen hat (Wurf im Render-Pfad reißt die halbe Seite mit). Für Lichess-FENs heute unkritisch, aber inkonsistent; dasselbe gilt für `base-puzzle-solver.ts:294` (`setupSolver`). Fix: `tryLoadFen` + `loadError`-Fallback, und die drei `reviewGoTo` in `BasePuzzleSolver` zusammenführen (passt zum geparkten Punkt „Cross-Solver-Duplikation").
- [ ] **Getter-Lawine in der Kursliste (LOW, Performance)**: `course-list.component.ts` wertet pro Change-Detection-Zyklus **8 Getter** aus (`@if` + `@for` je Sektion), von denen jeder `filtered` (voller Scan + `toLowerCase` pro Kurs) plus weitere Filter neu berechnet — bei Default-CD also bei jedem Event/Timer-Tick. Fix: einmal bei Änderung von `courses`/Suchtext berechnen (oder `computed()`-Signals). Generell: nur **10 von ~102** Komponenten nutzen `OnPush` (siehe geparkter Punkt „OnPush ausrollen").
- [ ] **81× `error: () => {}` (INFO)**: viele Ladefehler sind bewusst still (Poll-Feeds, Badges) — aber die Grenze ist nirgends dokumentiert. Wert wäre eine kurze Regel („still nur bei Poll/optionaler Anzeige, sonst Snackbar") statt einer Fallunterscheidung pro Stelle.
...
- [ ] **DSGVO-Restdaten nach Kontolöschung (LOW)**: persönliche Bücher (`Book.OwnerUserId`, inkl. importierter Kursinhalte) + zugehörige `CourseShares` und die Freitexte in `AdminMessages` überleben die Anonymisierung — Repertoires werden dagegen gelöscht. Entweder mitlöschen oder die Ausnahme im Doc-Kommentar begründen.
- [ ] **Kontolöschung per Username-Squatting blockierbar (LOW)**: `DeleteAccountAsync` setzt `deleted_{id}` / `deleted_{id}@deleted.invalid`; beides ist vorab registrierbar (kein Username-Pattern, nur Länge 3–50) → Unique-Index-Verletzung → Löschung schlägt dauerhaft mit 500 fehl. Fix: Zufallssuffix bzw. Kollisions-Retry.
- [ ] **OG-Renderer nutzt Host-Header (LOW)**: `OgController.BaseUrl()` baut `og:url`/`og:image` aus `Request.Host` + `X-Forwarded-Proto`, Antwort mit `Cache-Control: public, max-age=300` → Cache-Poisoning-Fläche hinter einem CDN. Fix: `App:BaseUrl` verwenden (existiert für Reset-Links).
...
- [ ] **BotStats-HMAC ohne Timestamp weiter akzeptiert (INFO)**: rückwärtskompatibler Zweig ⇒ unbegrenzt replaybar. Sobald der Bot `X-Bot-Timestamp` durchgängig schickt: Pflicht machen.
- [ ] **`ct` wird in `ClaudeJsonClient` nicht durchgereicht (LOW)**: `_client.Messages.Create(parameters)` ohne CancellationToken → Tipp-Generierung nicht abbrechbar (Shutdown/Abort).
- [ ] **Modell-Pin (INFO)**: `Model.ClaudeOpus4_8` ist weiter aktiv; `claude-opus-5` ist inzwischen der empfohlene Default bei identischem Preis ($5/$25 pro MTok) → Ein-Zeilen-Upgrade, optional `fallbacks: "default"` gegen Classifier-Refusals. Der Aufruf selbst ist aktuell (adaptive Thinking, structured output, refusal-Handling vor Content-Zugriff).
- [ ] **`/api/courses` aggregiert die ganze `BookPuzzles`-Tabelle (MITTEL, Performance)**: `puzzleCountByBook` ohne Filter auf die sichtbaren Bücher → Full-Scan je Aufruf der Kursliste. Fix: `.Where(bp => bookIds.Contains(bp.BookId))`. (Bei 49 Prod-Usern noch unkritisch, skaliert aber mit der Buchzahl, nicht der Userzahl.)
...
- [ ] **`/api/training-goals/daily-series` ohne Cache (MITTEL, Performance)**: aggregiert die komplette Historie über 8 Tabellen pro Request (bewusst ungedeckelt) — ein Kurz-Cache wäre billig. (Siehe auch die 2026-06-30-Bewertung: real max ~1355 Zeilen, daher weiter unkritisch.)
