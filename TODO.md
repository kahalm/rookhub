...
- [ ] LOW `target="_blank"` ohne `rel`: **erledigt (0.117.1)** für `tournament-detail.component.html`, `public-tournament.component.html` (rel ergänzt) + `chessable.component.ts` (`noopener` → `noopener noreferrer`). NOCH OFFEN: `navigator.clipboard.writeText` ohne `.catch()` (`api-tokens.component.ts:107`); `stopImpersonation()` `JSON.parse` ohne try/catch (`auth.service.ts:75`); diverse `Observable<any>` für Crawler-Job/Monitor-Responses.
