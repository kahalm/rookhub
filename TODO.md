...
- [ ] Hint-Nutzung granular tracken — **jedes** Hint-Holen einzeln festhalten (welches Puzzle, welche Stufe 1/2/3, User, Zeitpunkt), nicht nur das Maximum `HintsUsed` am Attempt-Ende (an `BookPuzzleAttempt`/`CourseAttempt`, seit v0.169.0). Zweck: auswerten, welche Puzzles wie oft/tief Tipps brauchen. Frontend-Hook ist `book-puzzle.component.showNextHint()` (löst je Stufe ein Event aus). Optionen: (a) strukturiertes Serilog-Event „HintFetched" via `POST /api/client-log` o. dediziertem Endpoint → ES/Kibana (kein DB-Schema); (b) eigene Tabelle `HintUsages`.
