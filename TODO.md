...
- [~] **Kleinere Performance-Fallen (LOW)** — **1 von 4 erledigt (v0.377.1):** `GamesController.List` zog ueber `CountPlies(g.Pgn)` in der Projektion die LONGTEXT-PGNs aller (bis 500) Partien, obwohl das DTO gar kein PGN traegt — `CountPlies` ist eine C#-Methode, EF kann sie nicht uebersetzen und holt die Spalte deshalb mit. Zaehler jetzt als nullbare Spalte `SavedGame.MoveCount` persistiert (Migration `SavedGameMoveCount`); das PGN wandert nur noch fuer Zeilen mit `MoveCount IS NULL` ueber die Leitung und wird dabei einmalig nachgetragen. SQL per `ToQueryString()` gegen den echten MySQL-Provider belegt. +3 Tests, gegengeprueft. **OFFEN (3):** `GetImportedOidsAsync` laedt alle Repertoire-PGNs + Regex im Request-Pfad eines Extension-Polls; `AppendLiveAsync` dedupliziert per `existing.Contains(moves)` ueber das ganze PGN (quadratisch) und setzt `FileSize` in Zeichen statt Bytes; `BookPuzzleService.ImportAsync` laedt alle `LineId`s der DB in ein HashSet.
