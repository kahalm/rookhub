...
- [~] **Test-Lücke** → **teilerledigt v0.184.11**: Specs für `menu.service`/`preferences.service`/`chessable.service`/`admin.service` + `profile.component` ergänzt (~35 Tests). OFFEN: `admin.component` (732 LOC, 6 Domänen) ohne Spec — größter verbleibender Gap.
- [~] **Klickbare `<div>`/`<span>`/`<mat-icon>`** ohne Tastatur → **teilerledigt v0.184.13**: `theme-chips` → `<button>`, `endless-history`-Mobil-Karte → role/tabindex/keydown(Enter+Space)+focus-visible. OFFEN: tournament-favoriten, repertoire-tree/-lines, puzzle-tags.
