...
## Periodisch
- [ ] **Unerwartete Chessable-Antworten aus RepCheck prüfen** (seit 0.478.8 / RepCheck 1.60.0) — letzter: **nie**.
  RepCheck stoppt „Kurs holen“, sobald Chessable etwas anderes liefert als erwartet, und bittet den Nutzer, VOR einem
  erneuten Versuch auf Discord Bescheid zu geben. Jede solche Antwort landet als Warnung im Log. Durchsehen: ES/Kibana
  Prod (7-Tage-Fenster) nach `ChessableUnexpectedResponse` (Tags `chessable,extension,crawl`) plus Admin-Nachrichten,
  die mit „[RepCheck] Chessable-Sperre gemeldet“ beginnen. Je Meldung entscheiden: **Anti-Crawling** (403 oder
  HTML-Sperrseite, 429 nach allen Wiederholungen, Fehlertexte wie „banned“/„blocked“, plötzlich leere `{}`) → Tempo
  in RepCheck anpassen und die betroffenen Nutzer warnen; **harmlos** (geänderte Feldnamen, eine gesperrte Linie in
  einem Vorschau-Kurs) → `checkChessableResponse` in `repcheck/extension/lib/chessable-crawl.js` erweitern und dem
  Nutzer grünes Licht geben. Gilt nur für die Extension — das Userscript prüft nicht.
...
- [x] Puzzle-Board auf den gemeinsamen `PromotionPickerComponent` migriert — **erledigt v0.377.5.** `puzzle-board.component.ts` trug eine eigene Inline-Auswahl mit identischer Positions-, Grafik- und Ghost-Tap-Logik. Jetzt `<app-promotion-picker>` wie im Analysebrett; Sichtbarkeit haengt an `pendingPromotion`, beide Einstiegspfade (Normal + Viz) ermitteln nur noch die Farbe. Entfallen: `showPromotionOverlay`, `promotionFilePercent`, `promotionFromBottom`, `promotionPieces`, `promotionGuardUntil`, `PROMOTION_GUARD_MS`, `PIECE_NAMES`, `getPieceImage` + 1147 Zeichen CSS (−134/+39 Zeilen). Der befuerchtete Viz-Pfad war unkritisch: er unterscheidet sich NUR in der Farbermittlung (aus `actualFen` statt vom eingefrorenen Brett), die Positionierung war zeichengleich. Guard-Specs des Bretts entfielen (in der Picker-Spec abgedeckt), dafuer 3 Specs fuer die Anbindung.
...
- [ ] **Zeitzonen statt harter UTC-Tagesgrenzen.** `TrainingGoalService`, `BookPuzzleService`,
  ...
  auseinander.
- [x] **Integrationstests gegen echtes MariaDB + Migrations-Smoke-Test in der CI** — **erledigt v0.377.6.** Neues Projekt `tests/RookHub.Api.IntegrationTests`: faehrt die ECHTE Anwendung per `WebApplicationFactory<Program>` gegen ein Wegwerf-Schema hoch (echter DI-Container, echte Service-Registrierungen, echter Provider) — die Tests pruefen also die Abfragen, die der Code wirklich absetzt, statt einer nachgebauten Kopie. Enthaelt: alle 139 Migrationen auf leerem Schema, Modell-gegen-Snapshot-Abgleich (faengt die vergessene Migration), und Uebersetzungstests fuer Kursstatistik / Partienliste / Chessable-Kennungen. CI-Job `test-rookhub-integration` mit `mariadb:11`-Dienstcontainer; haengt ueber `docker.yml → needs: tests` automatisch im Freigabe-Tor fuer die Image-Pushes. Ohne `ROOKHUB_TEST_MYSQL` ueberspringen sich die Tests, `dotnet test` bleibt lokal ohne Docker gruen. **Nachgestellt und belegt:** eine C#-Methode im `Where` liess alle 2045 InMemory-Tests gruen und faellt hier sofort mit „could not be translated“.
...
- [x] **`ComputeStatsAsync` im Hot-Path (MITTEL, Performance)** — **komplett erledigt.** Buchgroessen-Haelfte v0.377.1 (Puzzle-Liste nicht mehr laden, `total`/Kapitelzahl als SQL-Aggregate). **Historien-Haelfte v0.384.1:** `CourseAttempts` werden nicht mehr zeilenweise geladen; eine Gruppierung je Puzzle mit korrelierter Unterabfrage (`ORDER BY AttemptedAt, Id LIMIT 1`) liefert Zeitsumme und Erst-Versuch — eine Zeile je Puzzle statt je Versuch. Uebersetzung UND Weitergabe des Reset-Filters in die Unterabfrage per `ToQueryString()` belegt, dazu 3 Integrationstests gegen echte MariaDB (Erst-statt-Bester, Reset-Grenze, Gleichstand entscheidet die Id); gegengeprueft: bei umgedrehter Sortierung fallen 2 davon. War bis v0.377.6 blockiert, weil EF InMemory das nicht absichern kann.
- [x] **Kleinere Performance-Fallen (LOW)** — **alle 4 erledigt.** (1) `GamesController.List` zog ueber `CountPlies(g.Pgn)` in der Projektion die LONGTEXT-PGNs aller (bis 500) Partien → Zaehler als nullbare Spalte `SavedGame.MoveCount` persistiert, PGN nur noch fuer `MoveCount IS NULL` (v0.377.1). (2) `GetImportedOidsAsync` las bei jedem Extension-Poll jedes Repertoire-PGN komplett + Regex → Kennungen in `RepertoireFile.ChessableOidsCache` gemerkt, **selbstinvalidierend ueber die PGN-Laenge** (`CHAR_LENGTH` in SQL) statt Pflege an jeder Schreibstelle — eine uebersehene Schreibstelle faellt so nicht durch falsche Overlay-Haken auf, sondern nur durch einen Neuaufbau (v0.377.3). (3) `AppendLiveAsync`: Dedup ueber `existing.Contains(moves)` + `existing += …` je Linie war quadratisch → HashSet der exakten Zugtexte; `FileSize` zaehlte Zeichen statt Bytes (v0.377.3). (4) `BookPuzzleService.ImportAsync` lud ALLE `LineId`s der DB → nur noch die des Imports, in 500er-Bloecken (v0.377.3). Alle Projektionen per `ToQueryString()` gegen den echten MySQL-Provider belegt (die Tests laufen gegen EF InMemory und saehen ein Uebersetzungsproblem nicht). +5 Tests, gegengeprueft.
- [~] **Struktur (LOW)** — **`CanAccessAsync` erledigt (v0.378.1):** die Kurs-Zugriffsregel stellte bis zu SIEBEN sequenzielle Abfragen, obwohl alle Zweige ODER-verknuepft sind; jetzt eine gefaltete `Books.AnyAsync(...)` nach dem Vorbild `BookAccess.ReadableBy`. **Gegen echte MariaDB gemessen** (Befehls-Interceptor im Integrationstest): vorher 7, jetzt 1 — die alte Fassung laesst den Test fallen. Laeuft an jedem Kurs-Endpunkt, allein `CourseService` ruft sie an 17 Stellen. **Import-Zustaende auf Enum erledigt (v0.380.1):** `ChessableImportStatus` (5 Werte) + `ChessableImportPhase` (7 Werte) statt Zeichenketten ueber 8 Dateien; Werte-Konverter im DbContext, daher OHNE Migration und ohne Datenaenderung — DB und API-Vertrag bleiben byte-gleich (Integrationstest liest die Rohspalte gegen). Der Compiler fand dabei sofort zwei echte Fehler: die Feld-Doku nannte 3 der 5 Status-Werte, und zwei Test-Fixtures legten `Status = "importing"` bzw. `"fetching"` an — das sind PHASEN, keine Status. **Optionale Ctor-Parameter erledigt (v0.380.2):** `CourseService`, `RepertoireService` und `ProfileService` bauten sich `NotificationService`/`FriendService`/`BookAdminService` selbst, wenn der Aufrufer sie weglaesst — ausdruecklich „damit bestehende Test-Konstruktionen kompilieren". Alle 46 Aufrufstellen lagen in TESTS, im Produktivcode keine einzige. Jetzt verpflichtend + `TestServices`-Fabrik; laengste Konstruktionszeile 527 → 293 Zeichen. Ein Test fiel dabei zu Recht um: er teilt bewusst DIESELBE `RepertoireAnalyzeService`-Instanz, sonst prueft er zwei getrennte Caches — die Fabrik reicht sie jetzt durch. **OFFEN (1):** God-Services (`ChessableImportService` 1174, `CourseService` 1139, `TrainingGoalService` 962 Zeilen — seit dem Eintrag GEWACHSEN; grosser Diff, zahlt sich erst beim naechsten Anfassen aus); optionale Konstruktor-Parameter „fuer Tests“, die sich Abhaengigkeiten selbst bauen (`new NotificationService(db)` in `CourseService`/`RepertoireService`) statt DI/Fakes
...
- [ ] Anonyme Puzzle-Serie beim Registrieren bewerten (Elo aus geclaimter Sitzung) — `POST /api/puzzles/claim-session` (analog `book-puzzles`/`endless`) übernimmt die anonymen Versuche ins frisch angelegte Konto, rechnet aber **keine Elo**: die übernommenen Zeilen behalten `EloAfter`/`EloChange` NULL, `AppUsers.PuzzleElo` bleibt auf dem Startwert 1500. Live gesehen **2026-09-05** (Prod, User 113 `passsedpawn97`): 21:12-21:15 anonym **13/13 Puzzles gelöst** (Ratings 1404-1577, meist 2-14 s), 21:21 registriert — das Konto startet trotzdem mit Elo 1500 und leerer Puzzle-Statistik, obwohl der Beleg für die Einstufung unmittelbar davor geliefert wurde. Zu tun: die geclaimten Versuche beim Übernehmen chronologisch durch dieselbe Elo-Rechnung schicken wie reguläre Versuche (inkl. `PuzzleService.ProvisionalKFactor`, siehe Eintrag darüber) — Minimallösung sonst: vor dem anonymen Lösen sichtbar machen, dass diese Versuche nicht für die Wertung zählen.
- [~] Chessable-Repertoire ohne Extension: stille Sackgasse + irreführender 400er — Live gesehen **2026-09-05** (Prod, User 113): Repertoire „Nimzoindisch für Schwarz“ mit `ChessableCourseId=66522` angelegt, aber ohne verknüpften Bearer (`ChessableCredentials` für den User leer) bleibt es leer — keine `RepertoireFiles`, kein `ChessableImports`-Job, und nichts erklärt warum. Der Nutzer drückte daraufhin „Aktualisieren“ (`POST /api/repertoires/reprocess` → 202, ohne Dateien ein No-op) und dann „In Kurs umwandeln“ → `POST /api/repertoires/{id}/convert-to-course` **400** mit „The file does not look like a valid PGN.“ (`CourseService.UploadPersonalCourseAsync`, `src/api/RookHub.Api/Services/CourseService.cs:694`; die Nachbar-Meldung „No playable lines found in the PGN.“ :711 trifft es genauso wenig). Der nächste Klick des Nutzers war `/help`. Zu tun: (a) ein leeres Repertoire als leer kennzeichnen und den Weg über die RepCheck-Extension direkt anbieten, (b) `convert-to-course` bei leerem Repertoire eine eigene, verständliche Meldung geben statt der PGN-Fehlermeldung, (c) klären, ob ein Repertoire mit `ChessableCourseId` ohne hinterlegte Credentials überhaupt anlegbar sein soll. **(b) erledigt in v0.399.2**: `ConvertToCourse` unterscheidet den Leer-Fall jetzt per `code = "repertoire_empty"` im 400er, das Frontend zeigt dafür `repertoire.list.convertToCourseEmpty` (en/de/hr) mit den zwei Wegen zum Füllen — PGN hochladen oder Chessable-Kurs über RepCheck holen. **(a) und (c) offen.**
