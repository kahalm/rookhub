...
- [~] **`ComputeStatsAsync` im Hot-Path (MITTEL, Performance)** — **Buchgrößen-Hälfte erledigt v0.376.3:** die vollständige Puzzle-Liste des Buchs wird nicht mehr geladen; `total` und die Kapitelzahl sind SQL-Aggregate (Normalisierung leer/whitespace→null im Ausdruck mit übersetzt), Puzzle-Ids nur noch fürs angezeigte Kapitel. SQL-Übersetzung gegen den echten MySQL-Provider per `ToQueryString()` belegt, inkl. Null-Semantik (`IS NULL` statt `= NULL`). +1 Test, gegengeprüft. **OFFEN — und blockiert:** die Historien-Hälfte (`CourseAttempts` werden weiter zeilenweise geladen). Die Erst-Versuch-Auswertung nach SQL zu schieben braucht `GroupBy(...).OrderBy(...).First()`, und **alle 2036 API-Tests laufen gegen EF InMemory** — eine nicht übersetzbare Abfrage wäre dort grün und stürbe erst in Produktion. **Blockade seit v0.377.6 weg:** die Integrationstests laufen jetzt gegen echte MariaDB, eine nicht uebersetzbare Abfrage faellt dort sofort auf. Die Historien-Haelfte laesst sich damit angehen.
