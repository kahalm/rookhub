...
### rookhub API
- [~] HIGH `EncryptionService`: **rookhub erledigt (0.176.2)** — AES-GCM + `SHA256(key)` + `TryDecrypt` + Längen-Guard; Alt-CBC bleibt rückwärtskompatibel lesbar (kein Migration). Call-Sites (ChessableController/ChessableImportService) nutzen `TryDecrypt` → kein 500 mehr bei Rotation. **OFFEN:** die **duplizierte Klasse in piratechess `EncryptionService`** identisch härten (eigenes Repo/Deploy).
...
- [~] MED JWT wird bei Passwort-Reset/-Change + Account-Löschung nicht invalidiert (`AuthService.cs`, `Program.cs:90`) — alte Tokens bis 30 Tage gültig. **Account-Löschung erledigt (0.176.1):** JWT-`OnTokenValidated` prüft `DeletedAt` via `AuthUserValidation.IsActiveUserAsync` (IMemoryCache 60 s) → `ctx.Fail`; `ApiTokenAuthenticationHandler` lehnt gelöschte Owner ab; `DeleteAccountAsync` widerruft `UserApiTokens`. **OFFEN:** Passwort-Reset/-Change invalidieren bestehende JWTs noch nicht — bräuchte SecurityStamp/TokenVersion-Claim + Abgleich (Stamp rotieren bei Reset/Change). Grandfathering für Alt-Tokens (ohne Stamp-Claim) bedenken, sonst Massen-Logout beim Deploy.
...
- [ ] Kapitel-Spoiler dauerhaft entschärfen — Puzzle-Bücher zeigen Kapitelnamen wie „Chapter 2: Back-Rank Mates", die das Motiv verraten. Aktuell **manuell** nach jedem (Re-)Import via `scripts/hints/1001_deadly_checkmates.sql` (Re-Import überschreibt `bp.Chapter` → muss erneut laufen). Dauerhaft: beim Import für `Kind=Puzzle` den Teil nach „Chapter N:" strippen (`PgnImportService`/Chessable-Parse) **oder** im Frontend für Puzzle-Kurse nur „Chapter N"/Index zeigen. Entscheidung: nur Puzzle-Bücher (Study behalten ihre Kapitelnamen). Gilt generisch für alle Puzzle-Bücher, nicht nur „1001 Deadly Checkmates".
